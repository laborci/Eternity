...
class LoggerDeprecated implements SharedService {
	...
	public function sql(\PDOStatement $statement){
		$this->writeOut(' SQL ', $statement->queryString, 'sql');
	}
	...
	public function __invoke($message) {
		if (is_object($message) && get_class($message) === Request::class) {
			$this->request($message);
		} elseif (is_object($message) && get_class($message) === \PDOStatement::class) {
			$this->sql($message);
		} else {
			$this->info($message);
		}
